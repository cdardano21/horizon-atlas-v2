...
const buildEditorialText = (destination: Destination, researchProfile?: DestinationResearchProfile, premiumEditorialContent?: PremiumEditorialContent, knowledgeProfile?: DestinationKnowledgeProfile) => {
  ...
  const destinationDailyLife = normalize((destination as Destination & { dailyLife?: string }).dailyLife);
  const destinationHealthcareNarrative = normalize((destination as Destination & { healthcare?: string }).healthcare);
  const hasCustomResearchProfile = Boolean(researchProfile && Object.keys(researchProfile).length > 0);
  const hasKnowledgeProfile = Boolean(knowledgeProfile && Object.keys(knowledgeProfile).length > 0);
  const hasPremiumEditorialContent = Boolean(premiumEditorialContent && Object.keys(premiumEditorialContent).some((key) => Boolean((premiumEditorialContent as Record<string, unknown>)[key])));
  const fallbackToDestinationSpecificFields = !hasCustomResearchProfile && !hasKnowledgeProfile && !hasPremiumEditorialContent;
  const dailyLifeNarrative = fallbackToDestinationSpecificFields
    ? destinationDailyLife || normalize(resolvedResearchProfile?.feel) || lifestyle
    : normalize(resolvedResearchProfile?.feel) || normalize(resolvedResearchProfile?.whyPeopleLoveIt) || destinationDailyLife || lifestyle;
  const climateNarrative = fallbackToDestinationSpecificFields
    ? climate || normalize(resolvedResearchProfile?.climate)
    : normalize(resolvedResearchProfile?.climate) || climate;
  const transportationNarrative = normalize(premiumEditorialContent?.transportationArticle)
    || normalize(knowledgeProfile?.publicTransportation || knowledgeProfile?.localTransportation)
    || (fallbackToDestinationSpecificFields ? transportation : normalize(resolvedResearchProfile?.transportation) || transportation);
  const costNarrative = normalize(resolvedResearchProfile?.costOfLiving);
  const healthcareNarrative = normalize(premiumEditorialContent?.healthcareArticle)
    || (fallbackToDestinationSpecificFields ? destinationHealthcareNarrative : destinationHealthcareNarrative || normalize(resolvedResearchProfile?.healthcare))
    || normalize(resolvedResearchProfile?.healthcare);
  ...
  const healthcareGuide = premiumEditorialContent?.healthcareArticle?.trim() || (healthcareNarrative
    ? `${healthcareNarrative} A resident will care less about abstract system prestige than about how quickly they can reach a clinic, what specialist access looks like, and whether emergency care feels manageable from the chosen district. ${hospitals ? `The presence of ${hospitals} matters because it makes a long stay less dependent on outside referral patterns.` : "A local hospital network matters because it makes a long stay less dependent on outside referral patterns."} The strongest healthcare case comes from a household that pairs access to practical medical services with a home base that avoids unnecessary travel and keeps everyday errands close by.`
    : `${city} should be evaluated for healthcare through the lens of the actual weekly routine. ${healthcareQuality} A resident will care less about abstract system prestige than about how quickly they can reach a clinic, what specialist access looks like, and whether emergency care feels manageable from the chosen district. ${hospitals ? `The presence of ${hospitals} matters because it makes a long stay less dependent on outside referral patterns.` : "A local hospital network matters because it makes a long stay less dependent on outside referral patterns."} The strongest healthcare case comes from a household that pairs access to practical medical services with a home base that avoids unnecessary travel and keeps everyday errands close by.`);
