...
export default function Navbar() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-[rgba(57,52,42,0.08)] bg-[linear-gradient(180deg,rgba(248,244,236,0.92),rgba(247,240,226,0.86))] shadow-[0_14px_34px_-24px_rgba(31,35,33,0.52)] backdrop-blur-2xl">
      <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-4 sm:h-[5.5rem] sm:px-6 lg:h-24 lg:px-8">
        ...
        <nav className="hidden items-center rounded-full border border-[rgba(57,52,42,0.1)] bg-[rgba(255,252,246,0.9)] px-2 py-1.5 shadow-[0_12px_30px_-24px_rgba(31,35,33,0.52)] lg:flex">
          {links.map((link) => (
            <Link
              key={link.label}
              href={link.href}
              data-testid={`navbar-link-${toTestIdToken(link.label)}`}
              className="rounded-full px-4 py-2 text-xs font-semibold tracking-[0.12em] text-[var(--atlas-muted)] transition hover:bg-[rgba(31,95,99,0.08)] hover:text-[var(--atlas-accent)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[rgba(31,95,99,0.35)] focus-visible:ring-offset-2 focus-visible:ring-offset-[rgba(248,244,236,0.78)]"
            >
              {link.label}
            </Link>
          ))}
