...
export default function SmartShortlistPrototype({ candidates: suppliedCandidates = smartShortlistCandidates, intelligence, affordabilityRecords = [] }: {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (saved.requireBeach || saved.beach === "OCEAN_COASTAL") return;
              ...
              {step === 3 && (
                <div className="max-w-3xl">
                  ...
                  <div className="mt-8 grid gap-8 sm:grid-cols-2">
                    <div><h3 className="text-lg font-semibold">Beach access</h3><p className="mb-3 mt-1 text-xs text-[var(--atlas-muted)]">Soft beach preferences use general categories, including lakes. Required selections need verified ocean/sea evidence. Nearby access must be within 45 minutes by road; lake access does not qualify.</p><div className="grid gap-2"><ChoiceButton active={!beach} onClick={() => setBeach(undefined)}>No preference</ChoiceButton><ChoiceButton active={beach === "DIRECT_ACCESS"} onClick={() => setBeach("DIRECT_ACCESS")}>Direct beach access (ocean or lake preference)</ChoiceButton><ChoiceButton active={beach === "NEARBY_OR_DIRECT"} onClick={() => setBeach("NEARBY_OR_DIRECT")}>Nearby beach access (within 45 minutes when required)</ChoiceButton><ChoiceButton active={beach === "OCEAN_COASTAL"} onClick={() => { setBeach("OCEAN_COASTAL"); setRequireBeach(true); }}>Ocean/sea beach destination</ChoiceButton></div></div>
              ...
              {step === 5 && (
                <div className="max-w-3xl">
                  ...
                  <div className="mt-8 grid gap-3">
                    {beach && beach !== "OCEAN_COASTAL" && <label className="flex items-center gap-3 border border-[var(--atlas-border)] bg-white p-4 text-sm font-semibold"><input type="checkbox" checked={requireBeach} onChange={(event) => setRequireBeach(event.target.checked)} /> {beach === "NEARBY_OR_DIRECT" ? "Require ocean beach access within 45 minutes" : "Require a verified ocean beach destination"}</label>}
                    {beach === "OCEAN_COASTAL" && <p className="border border-[var(--atlas-border)] bg-white p-4 text-sm font-semibold">Ocean/sea beach destination is a hard requirement. Lake, nearby-only, and missing evidence are excluded.</p>}
