...
const outputPath = path.join(repoRoot, "docs/destination-expansion-wave1-practical-info-input.json");
...
function main() {
  ...
  const output = {
    ...
    expectedRecordSchema: {
      practicalInfo: namedRecordSchemaDescription(),
    },
    destinations,
  };

  fs.writeFileSync(outputPath, `${JSON.stringify(output, null, 2)}\n`, "utf8");
  console.log(`Wrote Wave 1 practicalInfo input scaffold: ${path.relative(repoRoot, outputPath)}`);
}
