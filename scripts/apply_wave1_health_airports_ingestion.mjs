...
const inputPath = path.join(repoRoot, "docs/destination-expansion-wave1-health-airports-input.json");
const mergedPath = path.join(repoRoot, "supabase/generated-command-center-seeds-merged.json");
const reportPath = path.join(repoRoot, "docs/destination-expansion-wave1-health-airports-ingestion-report.json");

const categories = ["healthcareFacilities", "airports"];
...
function validateNamedRecord(record, slug, category, index, errors) {
  ...
  for (const destination of destinations) {
    ...
    for (const category of categories) {
      ...
      records.forEach((rawRecord, index) => {
        ...
        validateNamedRecord(record, slug, category, index, errors);
...
function main() {
  ...
  for (const destination of destinations) {
    ...
    const normalizedHealthcare = destination.healthcareFacilities.map(normalizeNamedRecord);
    const normalizedAirports = destination.airports.map(normalizeNamedRecord);

    merged[slug] = {
      ...existing,
      healthcareFacilities: normalizedHealthcare,
      airports: normalizedAirports,
    };

    applied.push({
      slug,
      healthcareFacilitiesRecords: normalizedHealthcare.length,
      airportsRecords: normalizedAirports.length,
    });
  }
  ...
  const report = {
    ...
    totals: {
      healthcareFacilitiesRecords: applied.reduce((sum, item) => sum + item.healthcareFacilitiesRecords, 0),
      airportsRecords: applied.reduce((sum, item) => sum + item.airportsRecords, 0),
    },
    applied,
  };

  fs.writeFileSync(reportPath, `${JSON.stringify(report, null, 2)}\n`, "utf8");
  console.log(`Applied Wave 1 health/airports ingestion to merged seeds: ${path.relative(repoRoot, mergedPath)}`);
