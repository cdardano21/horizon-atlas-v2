...
const outputPath = path.join(repoRoot, "docs/destination-expansion-wave1-visa-tax-input.json");
...
function main() {
  ...
  const output = {
    ...
    expectedRecordSchema: {
      visaPrograms: namedRecordSchemaDescription(),
      taxRules: namedRecordSchemaDescription(),
    },
    destinations,
  };

  fs.writeFileSync(outputPath, `${JSON.stringify(output, null, 2)}\n`, "utf8");
  console.log(`Wrote Wave 1 visa/tax input scaffold: ${path.relative(repoRoot, outputPath)}`);
}
