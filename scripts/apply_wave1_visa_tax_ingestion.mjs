...
const inputPath = path.join(repoRoot, "docs/destination-expansion-wave1-visa-tax-input.json");
const mergedPath = path.join(repoRoot, "supabase/generated-command-center-seeds-merged.json");
const reportPath = path.join(repoRoot, "docs/destination-expansion-wave1-visa-tax-ingestion-report.json");

const categories = ["visaPrograms", "taxRules"];
...
function main() {
  ...
  for (const destination of destinations) {
    ...
    const normalizedVisa = destination.visaPrograms.map(normalizeNamedRecord);
    const normalizedTax = destination.taxRules.map(normalizeNamedRecord);

    merged[slug] = {
      ...existing,
      visaPrograms: normalizedVisa,
      taxRules: normalizedTax,
    };

    applied.push({
      slug,
      visaProgramsRecords: normalizedVisa.length,
      taxRulesRecords: normalizedTax.length,
    });
  }
  ...
  const report = {
    ...
    totals: {
      visaProgramsRecords: applied.reduce((sum, item) => sum + item.visaProgramsRecords, 0),
      taxRulesRecords: applied.reduce((sum, item) => sum + item.taxRulesRecords, 0),
    },
    applied,
  };

  fs.writeFileSync(reportPath, `${JSON.stringify(report, null, 2)}\n`, "utf8");
  console.log(`Applied Wave 1 visa/tax ingestion to merged seeds: ${path.relative(repoRoot, mergedPath)}`);
